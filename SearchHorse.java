...
public class SearchHorse extends Search{
    ...
    void EntryRaceInfoDisplay(ResultSet rs){
        //SELECT dete,race.name,jockey.name,bracketNum,horseRank,time 
		try {
            System.out.println("\n出走馬情報");                
            ...
            while(rs.next()){
                ...
                System.out.println(date+"\t"+PlusSpace(raceName,15)+"\t"+PlusSpace(jockeyName,10)+"\t"+bracketNum+"\t"+horseRank+"\t"+(int)(time/60)+":"+String.format("%02d"+"."+"%1d",(int)(time%60),(int)((time-(int)time)*10)));
            }
		} catch (SQLException se) {
			System.out.println("SQL Error: " + se.toString() + " "
				+ se.getErrorCode() + " " + se.getSQLState());
